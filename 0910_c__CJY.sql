...
select 고객이름, 나이, 직업, 적립금, 고객아이디 from 고객;
...
--가상컬럼명을 설정
--제품테이블에서 제조업체의 중복데이터를 제거하고 검색하시오.(단, 제조업체를 대표업체로 컬럼명을 변경)
select distinct 제조업체 as 대표업체 from 제품;
select distinct 제조업체 대표업체2 from 제품;

--제품테이블에서 제품명, 단가를 검색하되, 단가를 가격이라는 이름으로 출력
select 제품명, 단가 as 단가 from 제품;

--제품테이블에서 단가를 검색하되, 단가에 500원을 더해서 조정 단가라는 가상컬럼명으로 출력하시오.
